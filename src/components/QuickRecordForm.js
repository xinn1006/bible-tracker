...
export class QuickRecordForm {
  ...
  init() {
    ...
    this.container.addEventListener('change', (e) => {
      ...
      if (e.target.id === 'record-chapter') {
        this.selectedChapter = parseInt(e.target.value, 10);
      }
  ...
  updateChaptersDropdown() {
    const chSelect = document.getElementById('record-chapter');
    if (!chSelect) return;

    const bookMeta = BIBLE_BOOKS.find(b => b.name === this.selectedBook);
    const maxChapters = bookMeta ? bookMeta.chapters : 1;

    let options = '';
    for (let i = 1; i <= maxChapters; i++) {
      options += `<option value="${i}" ${i === 1 ? 'selected' : ''}>第 ${i} 章</option>`;
    }
  ...
  render({ isReadOnly = false } = {}) {
    const bookOptions = BIBLE_BOOKS.map(book => `
      <option value="${book.name}" ${book.name === this.selectedBook ? 'selected' : ''}>${book.name}</option>
    `).join('');

    const activeBookMeta = BIBLE_BOOKS.find(b => b.name === this.selectedBook);
    const maxChapters = activeBookMeta ? activeBookMeta.chapters : 1;
    let chapterOptions = '';
    for (let i = 1; i <= maxChapters; i++) {
      chapterOptions += `<option value="${i}">第 ${i} 章</option>`;
    }
    ...
    this.container.innerHTML = `
      <div class="record-form-container glass-panel ${isReadOnly ? 'readonly-mode' : ''}" style="height: 100%;">
        ...
        <!-- Bible Book and Chapter Details (Visible only when '讀經' is checked) -->
        <div id="bible-details-select" style="display: none; animation: fadeIn 0.2s ease;">
          ...
          <div class="form-group">
            <label for="record-chapter">讀經章節</label>
            <select id="record-chapter" class="input-control">
              ${chapterOptions}
            </select>
          </div>
        </div>
